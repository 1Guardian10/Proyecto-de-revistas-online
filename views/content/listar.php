...
<body>
    <main>
        <h1 class="text-3xl font-bold mb-6 dark:text-gray-800 text-center pt-5">Listado de Noticias</h1>

        <div class="grid grid-cols-1 gap-4 p-10">
            <?php
            if ($contenidos) {
                foreach ($contenidos as $noticia) {
                    echo '<a href="/public/ver?id=' . $noticia['id'] . '&t=' . $noticia['plantilla_id'] . '" class=" bg-white dark:bg-gray-700 p-4 rounded-lg shadow-lg hover:shadow-xl transition-shadow duration-200">';
            ...
            } else echo '<h2 class="text-3xl font-bold mb-6 dark:text-gray-800">Todavía no hay noticias</h2>';
            ?>
        </div>
    </main>
</body>
